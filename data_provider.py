...
class DataProvider():
    ...
    def __init__(self, config, verbose=False):
        ...
        self.path = config.data_dir + self.dataset+".txt"
        ...
        self.data, self.n_user, self.n_item = self.my_load_data(self.path)

        self.popular_item = self._cal_popular_item_set(min_count=5)

    def _cal_popular_item_set(self, min_count=5):
    ...
    def my_load_data(self, path, first=True):
        user_count = 0
        item_count = 0
        if first == False:  # If do not re-generate the dataset
            data_pack = np.load(path + "data_pack.npy", allow_pickle=True).item()
        ...
        if self.dataset == "ml100k" or self.dataset=="lastfm" or self.dataset=="douban_movie" or self.dataset=="steam":
            sequences = []
            with open(path, "r") as f:
                for line in f:
                    ...
                    temp_max_item = max(item_list)
                    if item_count < temp_max_item:
                        item_count = temp_max_item
    ...
    def _load_data(self, path, first=True, threshold=None):
        ...
        params:
            ...
            threshold: int, the minimum occurrence of item and user
        """
        if first == False:  # If do not re-generate the dataset
            data_pack = np.load(path + "data_pack.npy", allow_pickle=True).item()
            if self.verbose == True:
        ...
        if self.dataset == "ml-1m":
            ...
            data = pd.read_csv(rating_file, sep='\t', header=0, names=['userId', 'itemId', 'rating', 'timestamp'])
        ...
        sequences = []
        for userId, hist in data.groupby('userId'):  # group by 'userId'
            hist = hist.sort_values('timestamp').reset_index()  # sort the users' viewing history by timestamp,
            item_list = hist['itemId'].tolist()
            sequences.append(item_list)  # all items that user previously interacted with

        user_count = len(sequences)  # all individual user number
        item_set = data['itemId'].unique()  # all items in the system
        np.random.shuffle(item_set)
        item_count = len(item_set)  # all individual item number

        # Generate mapping of item id
        item_id = 1  # Note the item id mapping start from 1
        item_dict = {}
        item_reverse_dict = {}
        for i in item_set:
            item_dict[i] = item_id
            ...
            item_id += 1
        np.save(path + "item_dict.npy", item_dict)  # item_id---->index
        np.save(path + "item_reverse_dict", item_reverse_dict)  ##index---->item_id

        # Map item in the sequences to itemId
        for i in range(user_count):
            origin_seq = sequences[i]
            sequences[i] = [item_dict[j] for j in origin_seq]  # Mapping to item id
    ...
    def _nn1_split_data(self, data, seq_len=100, min_len=40):
        ...
        for user_id in range(len(data)):  # for every single user sequence
            ...
            seq = data[user_id][:-1]  # the remaining part is for accuracy testing
            length = len(seq)
            ...
            for i in range(sub_count):
                if i > 0 and i == sub_count - 1:  # more than two subsequences, and it is the last one
                    ...
                    end_idx = length
                elif i == 0 and i == sub_count - 1:  # The first subsequence is the last one
                    start_idx = 0
                    end_idx = length
                else:
                    start_idx = i * seq_len
                ...
                new_seq[:l_seq] = seq[start_idx:end_idx]  # Post-padding is used
                seqs.append([new_seq, user_id])  # Append the sequence and the corresponding user id
                seqs_count += 1
    ...
    def _random_split_data(self, data, seq_len=100, min_len=40, step=-1):
        ...
        seqs = []
        seqs_count = 0
        for user_id in range(len(data)):  # for every single user sequence; userid (0, len(data))
            # Generate training sequences
            seq = data[user_id][:-1]  # the remaining part is for accuracy testing
            ...
            while not stop:
                ...
                if remain_length <= min_len:  # If the remaining length is smaller than min_len
                    ...
                    start_idx = start_idx  # Starting position unchange
                    end_idx = np.random.randint(start_idx + min_len, start_idx + seq_len)
        ...
        if load == False:  # If do not load existing dataset
            ...
            if file_class == "irs":
                ...
                all_seqs = self._session_split_data(data=self.data,
                                                    ...
                                                    min_len=min_len,
                                                    step=step)
            elif file_class == "nn1":  ### split data for evaluator
            ...
            np.random.shuffle(all_seqs)  # 打乱用户顺序
            train_split_idx = int(len(all_seqs) * train_ratio)  # train_ratio=0.9 ?
            train_seqs = all_seqs[:train_split_idx]  # previous part as training seqs
            valid_seqs = all_seqs[train_split_idx:]  # subsequence part as training seqs
            ...
            if file_class == "irs":  # Save the targets that used in the training dataset
                ...
                np.save(self.path + "%s_targets.npy" % file_class, targets)  # irs_targets.npy
    ...
    def get_random_evaluate_data(self, seq_len=100, use_train=False, file_class="irs", gap_len=20, use_popular=True):
        """
        Process the sequence data for testing random target
        """
        if use_train == True and file_class == "irs":  # this branch
        ...
        #        targets = []
        #        labels = [] # Ground-truth labels for accuracy results
        #        new_seqs = []
        #        users = []
        eval_data = []
...
class DataLoaderEvalIRS(DataLoader):
    ...
    def _collate_fn(self, batch):
        ...
        targets = []
        labels = []
        ####(batch)   [his, user_id, random_targets, labels]
        for row in batch:
            seq = row[0]
            target = row[2]
            ...
            item_list = seq[-l_history:]
            start_history = -len(item_list) - self.gap_len - 1  # History start index
            new_seq[start_history:start_history + len(item_list)] = item_list  # Pre-padding
            new_seq[-1] = target  # Final new_seq: [0, 0, history, 0, 0, target]
    ...
    def _preprocess_seqs(self, histories, paths, targets, seq_len):
        ...
        for i in range(len(targets)):
            ...
            if pos == -1:  # Not early success
                path = np.concatenate((path, [target]))  # Append target directly to the tail
            ...
            if len(history) <= seq_len:  # If the history length is no larger than the input length
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default="ml-1m")  # 'lastfm', 'ml-1m'
