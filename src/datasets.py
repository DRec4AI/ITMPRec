...
class RecWithContrastiveLearningDataset(Dataset):
    ...
    def _data_sample_rec_task(self, user_id, items, input_ids, target_pos, answer):
        ...
        if self.test_neg_items is not None:
            # test_samples = self.test_neg_items[index]   ###original
            test_samples = self.test_neg_items[user_id]   ###modify
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == "train":
            ...
            input_ids = items[:-2]     #[0, 1, 2, 3, 4]
            target_pos = items[1:-1]   #[1, 2, 3, 4, 5]
            answer = [items[-2]]       #[5]
        else:   ####'test'
            items_with_noise = self._add_noise_interactions(items)
            input_ids = items_with_noise[:-1]   #[0, 1, 2, 3, 4, 5]
            target_pos = items_with_noise[1:]   #[1, 2, 3, 4, 5, 6]
            answer = [items_with_noise[-1]]     #[[6]]
        if self.data_type == "train":
