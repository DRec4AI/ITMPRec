...
def visual_nn1(input_seqs, derived_seqs, targets, id_dict, text_data):
    ...
    for i in range(len(derived_seqs)):
        ...
        genre = [list(tags[np.where(text_data.iloc[j - 1][tags].to_numpy().astype(int) == 1)]) for j in genre_path_id if
                 j != 0]
        length = len(path)
...
def get_feature_vector(dataset, datapath, use_fv=True):
    ...
    if dataset == "ml-100k":
        ...
        feature_dict = {key: to_onehot(text_data.loc[value][2].split('|'), str2num).astype(int) for key, value in
                        id_dict.items()}
    elif dataset == "lastfm":
